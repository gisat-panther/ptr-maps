...
var _uriTemplates = _interopRequireDefault(require("uri-templates"));
...
var WmtsLayer = /*#__PURE__*/function (_OsmLayer) {
  ...
  function WmtsLayer(layer) {
    ...
    _this.urlBuilder = _objectSpread(_objectSpread({}, _this.urlBuilder), {}, {
  ...
  _createClass(WmtsLayer, [{
    ...
    value: function urlForTile(url, tile, imageFormat) {
      var template = (0, _uriTemplates["default"])(url);
      ...
      if (template && template.varNames && template.varNames.length) {
        ...
        return "".concat(url, "/").concat(tile.level.levelNumber + 1, "/").concat(tile.column, "/").concat(tile.row, ".").concat(_webworldwindEsa["default"].WWUtil.suffixForMimeType(imageFormat));
      }
    }
  }]);
